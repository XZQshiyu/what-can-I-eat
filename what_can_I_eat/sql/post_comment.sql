...
BEGIN
    INSERT INTO post_comment (comment_id, post_id, user_id, context, publish_time, like_number)
    VALUES (p_comment_id, p_post_id, p_user_id, p_context, p_publish_time, p_like_number);
END //
delimiter ;
...
BEGIN
    UPDATE post_comment SET context = p_context WHERE comment_id = p_comment_id;
END //
delimiter ;
...
BEGIN
    DELETE FROM post_comment WHERE comment_id = p_comment_id;
END //
DELIMITER ;
